...
fn run() -> thrift::Result<()> {
    ...
    let mut thrift_test_client = {
        let (i_prot, o_prot) = build_stack(host, port, transport, protocol, "ThriftTest")?;
        ThriftTestSyncClient::new(i_prot, o_prot)
    };

    let mut second_service_client = if protocol.starts_with("multi") {
        let (i_prot, o_prot) = build_stack(host, port, transport, protocol, "SecondService")?;
...
// FIXME: expose "open" through the client interface so I don't have to early open
fn tcp_channel(
    host: &str,
    port: u16,
